...
router.patch("/:id/teacher-fields", async (req, res) => {
  ...
  try {
    ...
    if (workCertificate !== undefined) updateData.workCertificate = workCertificate;
...
router.post("/upload-work-certificate", upload.single("workCertificate"), (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({
        success: false,
...
router.post("/delete-diploma", (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      success: false,
      message: "Server error deleting diploma"
    });
  }
});
router.patch("/affectbadges/:idUser/:idBadge", async (req, res) => {
  ...
  try {
    ...
    const badgeExists = user.achievedBadges.some(badgeId => badgeId.toString() === idBadge);
    if (badgeExists) {
      return res.status(400).json({
...
router.get("/badges/:userId", async (req, res) => {
  ...
  const {
    User
  } = require("../models/usersModel"); // Import the User model
