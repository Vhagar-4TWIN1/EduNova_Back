...
const oauth2Client = new google.auth.OAuth2(process.env.GOOGLE_CLIENT_ID1, process.env.GOOGLE_CLIENT_SECRET1, 'http://localhost:3000/api/google/auth/callback' // ✅ Match frontend port
);
const SCOPES = ['https://www.googleapis.com/auth/classroom.courses.readonly', 'https://www.googleapis.com/auth/classroom.coursework.me', 'https://www.googleapis.com/auth/classroom.student-submissions.me.readonly'];
...
router.get('/lessons-temp', async (req, res) => {
  try {
        ...
        if (work.materials && work.materials.length > 0) {
          for (const material of work.materials) {
            if (material.driveFile) {
              ...
              const mimeType = (driveFile === null || driveFile === void 0 ? void 0 : driveFile.mimeType) || "";
    ...
    res.status(500).json({
      error: "Failed to fetch Classroom lessons"
    });
  }
});
router.post('/import-lessons', async (req, res) => {
  try {
    ...
    const {
      lessonIds
    } = req.body; // array of Google Classroom assignment IDs

    if (!Array.isArray(lessonIds) || lessonIds.length === 0) {
      return res.status(400).json({
        error: 'Invalid or empty lessonIds'
      });
    }
    oauth2Client.setCredentials(req.session.tokens);
    const classroom = google.classroom({
    ...
    for (const course of allCourses) {
      ...
      for (const work of courseWorksRes.data.courseWork || []) {
        if (lessonIds.includes(work.id)) {
          var _work$description;
          const newLesson = new Lesson({
            title: work.title || 'Untitled',
            content: ((_work$description = work.description) === null || _work$description === void 0 ? void 0 : _work$description.trim()) || 'No content provided by Google',
            typeLesson: 'text',
