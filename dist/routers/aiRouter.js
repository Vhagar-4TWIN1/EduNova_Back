...
const aiLimiter = rateLimit({
  ...
  // ✅ 10 seconds
  max: 2,
  // ✅ Allow up to 2 requests every 10s
  message: {
...
router.post("/dyslexia-feedback", aiLimiter, async (req, res) => {
  ...
  try {
    const response = await axios.post("https://api.openai.com/v1/chat/completions", {
      ...
      messages: [{
        role: "system",
        content: "You are a speech therapist assistant. Based on the user's speech, provide a short assessment of potential dyslexia-related challenges."
      }, {
        role: "user",
    ...
    if (error.response && error.response.status === 401) {
      ...
      return res.status(401).json({
        error: "Unauthorized access. Check API key."
      });
    }
    console.error("AI Dyslexia Feedback Error:", error.message);
    res.status(500).json({
      error: "Dyslexia feedback generation failed"
    });
  }
});
