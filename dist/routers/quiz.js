  ...
  try {
    ...
    const detailedResponses = await Promise.all(responses.map(async r => {
      ...
      return {
        ...
        // maintenant c’est bien un ObjectId ou null
        isCorrect
      };
    }));
    const correctCount = detailedResponses.filter(r => r.isCorrect).length;
    ...
    await attempt.save();
    const fullAttempt = await QuizAttempt.findById(attempt._id).populate({
      path: 'responses.question',
      populate: {
        path: 'level'
      }
...
const {
  User
} = require('../models/usersModel'); // Assurez-vous que c'est bien le bon chemin
  ...
  try {
    ...
    const studentsWithPerformance = await Promise.all(students.map(async student => {
      ...
      return {
        ...
        name: `${student.firstName} ${student.lastName}`,
        email: student.email,
        averageScore: lastAttempt.score.toFixed(2),
        // Utiliser le score de la dernière tentative
        totalAttempts: 1,
        // Une tentative présente
...
router.get('/global-stats', async (req, res) => {
  try {
    ...
    const globalAvgScore = ((_avgScoreResult$ = avgScoreResult[0]) === null || _avgScoreResult$ === void 0 ? void 0 : _avgScoreResult$.avgScore) || 0;
