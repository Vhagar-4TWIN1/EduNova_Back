...
passport.authenticateJWT = passport.authenticate("jwt", {
  session: false
});
...
passport.use(new GoogleStrategy({
  ...
  try {
    ...
    const downloadImage = async url => {
      try {
        const filename = `${Date.now()}-${Math.floor(Math.random() * 1000000000)}.jpg`; // Unique filename
    ...
    const country = profile.placesLived && profile.placesLived.length > 0 ? profile.placesLived[0].value // Extract first location
    ...
    if (!user) {
      ...
      user = new Student({
        ...
        verified: true,
        password: 'password',
        // à remplacer par un token temporaire ou rien du tout
        country: country || "Unknown",
        age: null,
        firstName: profile.name.givenName,
        lastName: profile.name.familyName,
        role: 'Student',
        // même si c'est redondant, utile pour cohérence
...
passport.use(new FacebookStrategy({
  ...
  try {
    ...
    const country = ((_profile$_json$locati = profile._json.location) === null || _profile$_json$locati === void 0 ? void 0 : _profile$_json$locati.name) || 'Unknown';
    if (!user) {
