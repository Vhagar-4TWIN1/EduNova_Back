...
const getAccessToken = async () => {
  try {
    const response = await axios.post(`${BLACKBOARD_BASE_URL}/learn/api/public/v1/oauth2/token`, 'grant_type=client_credentials', {
      headers: {
...
exports.createCourse = async (title, description) => {
  try {
    ...
    const res = await axios.post(`${BLACKBOARD_BASE_URL}/learn/api/public/v1/courses`, {
    ...
    if (error.response && error.response.status === 401) {
      ...
      return (void 0).createCourse(title, description);
    }
