...
const {
  diplomaVerificationController
} = require('./diplomaVerificationController');
...
exports.authenticate = (req, res, next) => {
  try {
    var _req$cookies$Authoriz, _req$headers$authoriz;
    const token = ((_req$cookies$Authoriz = req.cookies.Authorization) === null || _req$cookies$Authoriz === void 0 ? void 0 : _req$cookies$Authoriz.split(' ')[1]) || ((_req$headers$authoriz = req.headers.authorization) === null || _req$headers$authoriz === void 0 ? void 0 : _req$headers$authoriz.split(' ')[1]);
    if (!token) {
...
async function verifyDiploma(diplomaImage) {
  try {
    ...
    const mockReq = {
      file: {
        path: diplomaImage,
        // This should be the path to the uploaded image
    ...
    return {
      ...
      errors: {
        system: 'Verification process error'
      }
    };
  }
}
exports.signin = async (req, res) => {
  ...
  try {
    ...
    const activeSessions = await ActivityLog.find({
      ...
      action: 'LOGIN',
      createdAt: {
        $gte: new Date(Date.now() - 8 * 3600000)
      }
    });
  ...
  try {
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
      ...
      try {
        ...
        console.log("Utilisateur trouvé:", (_user = user) === null || _user === void 0 ? void 0 : _user.email);
      } catch (err) {
    ...
    if (user) {
      ...
      const duration = lastLogin ? Date.now() - lastLogin.createdAt.getTime() : null;
      await ActivityLog.create({
        userId: user._id,
        email: user.email,
        action: 'LOGOUT',
  ...
  try {
    ...
    for (const user of users) {
      ...
      const logs = await ActivityLog.find({
        userId: user._id,
        action: {
          $in: ['LOGIN', 'LOGOUT', 'PASSWORD_CHANGE', 'SIGNUP', 'CHECK_MODULE', 'CHECK_LESSON', 'LESSON_COMPLETED', 'WATCH_MUSIC', 'VIDEO_CALL', 'START_EVALUATION', 'SUBMIT_EVALUATION', 'CHECK_LESSON_DURATION', 'FORUM', 'REPLY_FORUM']
        }
      }).sort({
        createdAt: 1
...
exports.getUserSessionDuration = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: 'An error occurred while retrieving session duration.'
    });
  }
};
exports.getUserActionStats = async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: "Failed to get user action stats"
    });
  }
};