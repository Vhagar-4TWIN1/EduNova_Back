  ...
  try {
    ...
    const question = await Question.create({
      ...
      answers: parsedAnswers.map((answer, i) => ({
        ...
        audioUrl: questionType === 'oral' && files.answerAudios && files.answerAudios[i] ? files.answerAudios[i].path : null,
...
exports.updateQuestion = async (req, res) => {
  try {
    ...
    if (questionType === 'oral') {
      ...
      updates.answers = updates.answers.map((answer, i) => {
        ...
        if (files.answerAudios && files.answerAudios[i]) {
          if (oldAnswer !== null && oldAnswer !== void 0 && oldAnswer.audioUrl) {
    ...
    const updatedQuestion = await Question.findByIdAndUpdate(req.params.id, updates, {
      new: true,
      runValidators: true
    }).populate('level', 'name');
    res.json({
...
function generateRandomWrongAnswers(count) {
  const fakeAnswers = ["42", "Blue whale", "Mount Everest", "Water", "False", "Einstein", "Banana", "Paris", "10", "Gravity"];
  const selected = [];
    ...
    await new Promise((resolve, reject) => {
      fs.createReadStream(filePath).pipe(csv({
        ...
        try {
          ...
          let questionText = data.questionText && data.questionText.trim() !== '' ? data.questionText.trim() : "Generated Question Text " + Math.floor(Math.random() * 1000);
          const answers = [];
          let correctCount = 0;
          ...
          for (let i = 1; i <= 4; i++) {
            ...
            if (data[answerKey] && data[answerKey].trim() !== '') {
              const isCorrect = data[correctKey] ? data[correctKey].toString().toLowerCase() === "true" : false;
              ...
              answers.push({
                text: generateRandomWrongAnswers(1)[0].text,
                // Random wrong answer
                isCorrect: false
              });
            }
