...
} = require("./calendarEventController");
...
async function autoSchedule(req, res) {
  ...
  if (!moduleId) {
    return res.status(400).json({
      error: "Must provide moduleId"
    });
  }
  if (!dueDate) {
    return res.status(400).json({
      error: "Must provide dueDate"
    });
  }
...
${existingEvents.length ? existingEvents.map(ev => ` • ${ev.start.toISOString()} → ${ev.end.toISOString()}`).join("\n") : " • none"}
...
`.trim()
    ...
    if (!orRes.ok) {
      var _orJson$error;
      throw new Error(((_orJson$error = orJson.error) === null || _orJson$error === void 0 ? void 0 : _orJson$error.message) || `Status ${orRes.status}`);
    }
    rawReply = orJson.choices[0].message.content.trim();
  } catch (err) {
    console.error("AI scheduling error:", err);
    return res.status(502).json({
      error: "AI scheduling service failed"
    });
  }
  ...
  if (startIdx < 0 || endIdx < 0) {
    ...
    try {
      var _fixJson$choices, _fixJson$choices$, _fixJson$choices$$mes, _fixJson$choices$$mes2;
      ...
      const newReply = ((_fixJson$choices = fixJson.choices) === null || _fixJson$choices === void 0 ? void 0 : (_fixJson$choices$ = _fixJson$choices[0]) === null || _fixJson$choices$ === void 0 ? void 0 : (_fixJson$choices$$mes = _fixJson$choices$.message) === null || _fixJson$choices$$mes === void 0 ? void 0 : (_fixJson$choices$$mes2 = _fixJson$choices$$mes.content) === null || _fixJson$choices$$mes2 === void 0 ? void 0 : _fixJson$choices$$mes2.trim()) || "";
      rawReply = newReply;
  ...
  if (!created.length) {
    return res.status(400).json({
      error: "No valid slots scheduled"
    });
  }
