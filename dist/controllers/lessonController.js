...
const {
  generateAnnotations
} = require("../utils/generateAnnotations");
  ...
  try {
    ...
    if (mime.startsWith("audio/") || mime.startsWith("video/")) {
      ...
      await new Promise((resolve, reject) => {
        ffmpeg().input(localPath).audioChannels(1).audioFrequency(16000).format("wav").on("end", resolve).on("error", reject).save(wavPath);
      });
...
exports.trackLessonView = async (req, res) => {
  try {
    ...
    if (!lesson.module) {
      return res.status(400).json({
        message: 'Lesson is not associated with a module'
      });
    }
