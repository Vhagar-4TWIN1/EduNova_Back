...
exports.markLessonCompletedForEvent = async (req, res) => {
  ...
  try {
    ...
    const lessonEvents = await CalendarEvent.find({
      userId,
      lessonId,
      type: "lesson"
    });
    const allCompleted = lessonEvents.length > 0 && lessonEvents.every(ev => ev.completed);
    const alreadyAdded = progress.completedLessons.some(id => new mongoose.Types.ObjectId(id).equals(lessonId));
...
exports.markEventCompleted = async (req, res) => {
  ...
  try {
    ...
    const moduleDoc = await Module.findOne({
      "lessons._id": event.lessonId
    }, {
      _id: 1
    });
    if (!moduleDoc) return res.status(400).json({
      message: "Could not find module for this lesson"
    });
      ...
      case "lesson":
        {
          ...
          if (lessonEvents.length === 2 && lessonEvents.every(ev => ev.completed) && !progress.completedLessons.includes(event.lessonId)) {
    ...
    await progress.save();
    return res.json({
      message: `${event.type} marked as completed`
    });
  } catch (err) {
    console.error(err);
    return res.status(500).json({
...
exports.getAllProgressByUser = async (req, res) => {
  ...
  try {
    ...
    if (progresses.length) {
      completedLessons = progresses.reduce((sum, p) => {
        var _p$completedLessons;
        return sum + (((_p$completedLessons = p.completedLessons) === null || _p$completedLessons === void 0 ? void 0 : _p$completedLessons.length) || 0);
      }, 0);
      completedTasks = progresses.reduce((sum, p) => {
        var _p$completedTasks;
        return sum + (((_p$completedTasks = p.completedTasks) === null || _p$completedTasks === void 0 ? void 0 : _p$completedTasks.length) || 0);
      }, 0);
    } else {
      var _lessonCompletionAgg$;
      ...
      completedLessons = ((_lessonCompletionAgg$ = lessonCompletionAgg[0]) === null || _lessonCompletionAgg$ === void 0 ? void 0 : _lessonCompletionAgg$.completedLessons) || 0;
    ...
    const dateSet = new Set(completedEvents.filter(ev => ev.completedAt).map(ev => ev.completedAt.toISOString().slice(0, 10)));
    let streak = 0;
