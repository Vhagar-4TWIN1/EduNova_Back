...
exports.getAllUsers = async (req, res) => {
  try {
    ...
    const users = await User.find().select("-password").skip(skip).limit(limit).lean(); // Include all fields, including role and __t
...
exports.affectBadge = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "Badge affected successfully"
    });
  } catch (error) {
...
exports.evaluateAndAssignBadges = async userId => {
  try {
    ...
    const newBadges = badgeIdsToAssign.filter(badgeId => !student.achievedBadges.includes(badgeId));
