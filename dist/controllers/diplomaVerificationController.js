...
const preprocessImage = async imagePath => {
  try {
    ...
    await sharp(imagePath).resize(800).grayscale().normalize().sharpen().toFile(tempPath);
...
const cleanOCRText = text => {
  return text.replace(/Abderrabmase/g, 'Abderrahmane').replace(/BEJAI/g, 'Béjaia').replace(/ENSIGNANT/g, 'Enseignant').replace(/vacataire/g, 'vacataire').replace(/\s{2,}/g, ' ').trim();
};
  ...
  try {
    upload(req, res, async err => {
      ...
      try {
        ...
        } = await Tesseract.recognize(imagePath, 'fra');
