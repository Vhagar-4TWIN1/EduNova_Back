...
app.use(cors({
  origin: "*",
  // CORS autorisé pour le frontend React
  credentials: true,
...
app.get("/auth", async (req, res) => {
  ...
  try {
    var _emailsRes$data$find, _emailsRes$data$;
    ...
    const primaryEmail = ((_emailsRes$data$find = emailsRes.data.find(email => email.primary && email.verified)) === null || _emailsRes$data$find === void 0 ? void 0 : _emailsRes$data$find.email) || ((_emailsRes$data$ = emailsRes.data[0]) === null || _emailsRes$data$ === void 0 ? void 0 : _emailsRes$data$.email) || `${githubUser.login}@github.com`;
    let user = await User.findOne({
      email: primaryEmail
    });
    if (!user) {
      var _githubUser$name, _githubUser$name2;
      const randomPassword = Math.random().toString(36).slice(-8);
      const hashedPassword = await bcrypt.hash(randomPassword, 10);
      user = new User({
        firstName: ((_githubUser$name = githubUser.name) === null || _githubUser$name === void 0 ? void 0 : _githubUser$name.split(" ")[0]) || githubUser.login,
        lastName: ((_githubUser$name2 = githubUser.name) === null || _githubUser$name2 === void 0 ? void 0 : _githubUser$name2.split(" ")[1]) || "",
        email: primaryEmail,
    ...
    console.error("GitHub OAuth Error:", ((_err$response = err.response) === null || _err$response === void 0 ? void 0 : _err$response.data) || err.message || err);
    res.status(500).json({
      error: "GitHub OAuth failed",
      details: ((_err$response2 = err.response) === null || _err$response2 === void 0 ? void 0 : _err$response2.data) || err.message
    });
  }
});
