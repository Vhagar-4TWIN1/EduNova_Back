...
async function getAllEvents(req, res) {
  try {
    const { range, start, end, type } = req.query;
    const now = new Date();
    const today = new Date(now.setHours(0, 0, 0, 0));

    const query = {};
    if (start) query.start = { $gte: new Date(start) };
    if (end) query.end = { $lte: new Date(end) };
...
async function getEventsByUser(req, res) {
  try {
    ...
    const evs = await CalendarEvent.find({ userId })
...
async function getEventsByLesson(req, res) {
  try {
    ...
    const evs = await CalendarEvent.find({ lessonId }).sort({ start: 1 });
    return res.status(200).json(evs);
  } catch (err) {
    console.error('Error fetching lesson events:', err);
    return res.status(500).json({ error: 'Failed to fetch lesson events' });
  }
}
...
async function createEvent(req, res) {
  const { type, title, description, start, end, priority, lessonId, userId } = req.body;
...
async function updateEvent(req, res) {
  try {
    ...
    const ev = await CalendarEvent.findByIdAndUpdate(id, updates, { new: true });
...
function broadcastEventUpdate() {
  ...
  wss.clients.forEach(client => {
    if (client.readyState === WebSocket.OPEN) {
      client.send(JSON.stringify({ type: 'events:update' }));
    }
  });
}
    ...
    if (req.app.locals.wss) {
      req.app.locals.wss.clients.forEach(ws => {
        if (ws.readyState === WebSocket.OPEN) {
          ws.send(JSON.stringify({ type: 'events:update' }));
        }
      });
    }
    return res.status(200).json(ev);
