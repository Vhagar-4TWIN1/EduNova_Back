...
exports.getStickyNotes = async (req, res) => {
  try {
    ...
    const annotation = lesson.annotations.find(
...
exports.addStickyNote = async (req, res) => {
  try {
    const { lessonId } = req.params;
    const { userId, content, position, color } = req.body;
    ...
    let annotation = lesson.annotations.find(
      (a) => a.userId && a.userId.toString() === userId
    );
    ...
    if (annotation) {
      ...
      lesson.annotations.push({
        ...
        highlights: [], // ✅ ensure other schema fields exist
        notes: [],
        stickyNotes: [newNote],
      });
    }
    ...
    await lesson.save();
    res.status(201).json({ message: "Sticky note added successfully" });
  } catch (err) {
...
exports.updateStickyNote = async (req, res) => {
  try {
    const { lessonId, noteIndex } = req.params;
    const { userId, content, position, color } = req.body;
