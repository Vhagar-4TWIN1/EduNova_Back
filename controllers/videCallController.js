...
exports.createVideoCall = async (req, res) => {
    ...
    try {
        const videoCall = new VideoCall({ participants });
...
exports.getVideoCall = async (req, res) => {
    ...
    try {
        ...
        if (!videoCall) return res.status(404).json({ message: 'Video call not found' });
        res.json(videoCall);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
exports.getAllVideoCalls = async (req, res) => {
    try {
        const videoCalls = await VideoCall.find().populate('participants', 'username').sort({ createdAt: -1 });
        res.json(videoCalls);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
exports.updateVideoCall = async (req, res) => {
    ...
    const { participants } = req.body;
    try {
        const videoCall = await VideoCall
.findByIdAndUpdate(id, { participants }, { new: true });
        if (!videoCall) return res.status(404).json({ message: 'Video call not found' });
        res.json(videoCall);
    }
    catch (err) {
...
exports.addParticipant = async (req, res) => {
    ...
    try {
        ...
        if (!videoCall) return res.status(404).json({ message: 'Video call not found' });
        if (videoCall.participants.includes(userId)) return res.status(400).json({ message: 'User already in video call' });
