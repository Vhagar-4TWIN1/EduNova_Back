...
async function getSkillTree(req, res) {
  try {
    ...
    const nodes = await SkillNode.find({ lessonId }).sort({ 'position.x': 1 });
...
async function createSkillNode(req, res) {
  try {
    const { title, lessonId, parentId, position } = req.body;
    ...
    return res.status(500).json({ error: 'Failed to create skill node' });
  }
}
...
async function updateSkillNode(req, res) {
  try {
    ...
    const node = await SkillNode.findByIdAndUpdate(id, updates, { new: true });
...
async function deleteSkillNode(req, res) {
  try {
    ...
    broadcastSkillTreeUpdate(node.lessonId.toString());
    return res.status(200).json({ message: 'Skill node deleted successfully' });
  } catch (err) {
